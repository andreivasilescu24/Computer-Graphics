...
namespace implemented
{
    class Camera_t2
    {
     public:
        ...
        {
            // Translates the CameraTema2 using the `dir` vector computed from
            // `forward`. Movement will always keep the CameraTema2 at the same
        ...
        {
            // TODO(student): Translate the CameraTema2 using the `forward` vector.
        ...
        {
            // TODO(student): Translate the CameraTema2 using the `up` vector.
        ...
        void TranslateRight(float distance)
        {
            ...
            // You need to translate the CameraTema2 using the `right` vector.
            // Usually, however, translation using CameraTema2's `right` vector
            // is not very useful, because if the CameraTema2 is rotated around the
            // `forward` vector, then the translation on the `right` direction
            // will have an undesired effect, more precisely, the CameraTema2 will
        ...
        void RotateFirstPerson_OZ(float angle)
        {
            ...
            glm::vec4 aux = glm::rotate(glm::mat4(1.f), angle, forward) * glm::vec4(right, 1);
            right = glm::normalize(glm::vec3(aux));
            up = glm::cross(right, forward);
        ...
        {
            // TODO(student): Rotate the CameraTema2 in third-person mode around
