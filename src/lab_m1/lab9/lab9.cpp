...
void Lab9::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, Texture2D* texture1, Texture2D* texture2)
{
    ...
    if (mesh == meshes["sphere"])
        glUniform1f(location_time, (float)Engine::GetElapsedTime());
    else
        glUniform1f(location_time, -1.0f);

    glUniform1i(glGetUniformLocation(shader->program, "mix_textures"), mix);
    ...
    if (texture1)
    {
        ...
        int location_text1 = glGetUniformLocation(shader->program, "texture_1");
        glUniform1i(location_text1, 0);
    }
    ...
    if (texture2)
    {
        ...
        int location_text2 = glGetUniformLocation(shader->program, "texture_2");
        glUniform1i(location_text2, 1);
    }
